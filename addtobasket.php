...
$proteinname=$_REQUEST["proteinname"];

$finalproteinname="";
if($proteinname!=""){
	$finalproteinname=$proteinname;
}
...
$condimentsname=$_REQUEST["condimentsname"];

$finalcondimentsname="";
if($condimentsname!=""){
	$finalcondimentsname=$condimentsname;
}
...
if($final_selected_string!="")
{
	...
	for($a=0;$a<count($main_explode);$a++)
	{
		...
		if($second_explode[2]=="Protein")
		{
			$proteinselected_id=$second_explode[0];
		}
	}
}
...
for($i=0;$i<$counter;$i++)
{
	...
	if($data['itemChoiceInfo'][$i]['choiceCategory']=="Protien"){
		$Protein="1";
	}
...
$html.='<div class="inner-content">
					...
					<div class="listGroup">
						...
						if($grindercondiments=="1"){
							$html.='<div class="bar bar-default">
								<a href="#listingPage" onclick="show_condiments(\'' . $item_id . '\',\'' . $choiceLimit . '\');" class="ui-link">Condiments<br/><span id="food_condiments" style="font-size: 13px;">'.$finalcondimentsname.'</span></a>
							</div><input type="hidden" name="hasgrindercondiments" id="hasgrindercondiments" value="1" />';
						}else{
							$html.='<input type="hidden" name="hasgrindercondiments" id="hasgrindercondiments" value="0" />';
						}
						...
						if($Protein=="1"){
							...
							{
								$html.='<div class="bar bar-default">
									<a href="#listingPage" onclick="show_Protein(\'' . $item_id . '\',\'' . $choiceLimit . '\',\'' . $proteinselected_id . '\',\'' . $mode . '\');" class="ui-link">Protien<br/><span id="food_protein" style="font-size: 13px;">'.$finalproteinname.'</span></a>
								</div><input type="hidden" name="hasprotein" id="hasprotein" value="1" />';
							}
							else
							{
								$html.='<div class="bar bar-default">
								<a href="#listingPage" onclick="show_Protein(\'' . $item_id . '\',\'' . $choiceLimit . '\');" class="ui-link">Protien<br/><span id="food_protein" style="font-size: 13px;">'.$finalproteinname.'</span></a>
							 </div><input type="hidden" name="hasprotein" id="hasprotein" value="1" />';
							}
							
						}else{
							$html.='<input type="hidden" name="hasprotein" id="hasprotein" value="0" />';
						}
