...
$mode=$_REQUEST["mode"];
$selectedval=$_REQUEST["selectedval"];

$expl_selectedval=explode('======',$selectedval);
...
//echo "0--------"."http://whatthetrucks.com:8080/Innovommerce/rest/itemchoicelist/choices?itemid=".$item_id."&choiceLimit=".$choiceLimit."";
						...
						for($j=0;$j<count($choiceGroup1);$j++)
						{
							...
							if($group=="true"){
								...
								$html.='<div class="bar bar-default">
									<a href="#listingPage" onclick="show_radio_lists(\'' . $item_id . '\',\'' . $choiceLimit . '\',\'' . $name . '\');" class="ui-link">'.$name.'<br/><span style="font-size: 13px;" id="'.$name.'div">';

									for($x=0;$x<count($expl_selectedval);$x++){
										$elem=$expl_selectedval[$x];
										$expl_elem=explode(':', $elem);
										if($name==$expl_elem[0]){
											$html.=$expl_elem[1];
										}
								...
								$html.='<div class="bar bar-default">
									<a href="#listingPage" onclick="show_checkbox_lists(\'' . $item_id . '\',\'' . $choiceLimit . '\',\'' . $name . '\');" class="ui-link">'.$name.'<br/><span style="font-size: 13px;" id="'.$name.'div">';
									for($x=0;$x<count($expl_selectedval);$x++){
										$elem=$expl_selectedval[$x];
										$expl_elem=explode(':', $elem);
										if($name==$expl_elem[0]){
