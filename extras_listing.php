								...
								if($data['itemChoiceInfo'][$i]['choiceCategory']=="Extras")
								{
			                            $html.='<li>
			                                <div class="inputRadio">
			                                    <input type="checkbox" onclick="OnChangeCheckbox(this);" name="extraItem" id="'.$i.'Item" data-role="none" value="'.$data['itemChoiceInfo'][$i]['choicePrice'].'++'.$data['itemChoiceInfo'][$i]['choiceName'].'"/>
			                 ...
			                 else
			                 {
			                 	if($data['itemChoiceInfo'][$i]['choiceCategory']=="Extras")
								{
									...
									$explode=explode(",",$selected_items);
										 ...
										 $html.='<li>
			                               <div class="inputRadio">
			                                    <input type="checkbox" onclick="OnChangeCheckbox(this);" name="extraItem" id="'.$i.'Item" ';if(in_array($i.'Item', $explode)){ $html.=' checked '; } else { $html.=' '; } $html.=' data-role="none" value="'.$data['itemChoiceInfo'][$i]['choicePrice'].'++'.$data['itemChoiceInfo'][$i]['choiceName'].'"/>
			                                    <label for="'.$i.'Item">'.$data['itemChoiceInfo'][$i]['choiceName']; if($data['itemChoiceInfo'][$i]['choicePrice']!="0.0"){ $html.=' ( $'.number_format($data['itemChoiceInfo'][$i]['choicePrice'], 2).' )'; } $html.='</label>
			                                </div>
			                            </li>';
