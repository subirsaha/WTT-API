                        ...
                        for($i=0;$i<$counter;$i++)
						{
							...
							else
							{
								if($data['itemChoiceInfo'][$i]['choiceCategory']=="Choice")
								{
		                            $html.='<li>
		                                <div class="inputRadio">
		                                    <input type="radio" name="Item" id="'.$i.'Item" ';if($data['itemChoiceInfo'][$i]['choiceName']==$selected_val){ $html.=' checked '; } else { $html.=' '; } $html.=' data-role="none" onclick = "MyRadio2(\'' . $data['itemChoiceInfo'][$i]['choiceName'] . '\',\'' . $data['itemChoiceInfo'][$i]['id'] . '\',\'' . $data['itemChoiceInfo'][$i]['choicePrice'] . '\',\'' . $choice_type . '\')"/>
		                                    <label for="'.$i.'Item">'.$data['itemChoiceInfo'][$i]['choiceName'].' ( $'.$data['itemChoiceInfo'][$i]['choicePrice'].' )</label>
		                                </div>
		                            </li>';
		                     	}
