...
$json = file_get_contents("http://dookansserver.com/Innovommerce/rest/businessinfolist/listfoodtrucks?LATLONG=".$lat.",".$long."&name=".$keywrd."&sortBy=distance"); // this WILL do an http request for you
...
for($i=0;$i<$counter;$i++)
{
	...
	$html .= '<div class="bar bar-default">
						<div class="search-result-box" >
							<h3><a  onClick="gotoDetailsRes(\'' . $data['businessInfo'][$i]['id'] . '\',\'' . $lat . '\',\'' . $long . '\',\'' . $busId . '\',\'' . $data['businessInfo'][$i]['businessName'] . '\',\'' . $add . '\',\'' . $time_in_12_hour_format_start . '\',\'' . $data['businessInfo'][$i]['pickupTime'] . '\')" >'.$data['businessInfo'][$i]['id'].'******************'.$data['businessInfo'][$i]['businessName'].'</a></h3>
							...
							<div class="inline-icon">
								...
								$html .='<span class="icon-block order">WTT Orders</span>';
										...
										$html .='<span class="icon-block order">View Orders</span>';
								}
							$html .='</div>
